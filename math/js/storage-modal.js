/**
 * Storage and Modal Controller Module.
 * Handles modal dialogs, paste JSON imports, local file uploads, and browser LocalStorage.
 */
...
/** Applies user JSON from modal text area. */
...
/** Updates top header status indicator text. */
...
function saveToLocalStorage(data) {
  try {
    localStorage.setItem("GRADIFIED_MATH_SAVED_REPORT", JSON.stringify(data));
  } catch (err) {
...
function loadFromLocalStorage() {
  try {
    const savedString = localStorage.getItem("GRADIFIED_MATH_SAVED_REPORT");
    if (savedString) {
