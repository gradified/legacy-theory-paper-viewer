/**
 * Component builders for SVG visual dials, rubric score bars,
 * step breakdowns, MCQ cells, and question cards.
 */
 ...
 * @param {Array} rubricScores - List of rubric score items.
 * @returns {string} Rubric bar HTML string.
 */
...
/**
 * Builds step-by-step scoring breakdown list.
...
function buildStepsBreakdown(stepsAnalysis) {
  ...
  const stepRows = stepsAnalysis.map(stepItem => {
    ...
    const markLabel = isEarned ? `✓ +${stepItem.marks || 1}` : '✗ 0';
    return `
    <div class="step-row">
...
/**
 * Builds Multiple Choice Question grid cell cards.
 * @param {Array} mcqResults - List of normalized MCQ objects.
 * @returns {string} MCQ grid HTML string.
 */
...
/**
 * Builds subjective question evaluation card.
 * @param {Object} question - Normalized question item.
